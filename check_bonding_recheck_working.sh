...
cat $BOND_DIR/$i | grep "Slave Interface: "| cut -d ":" -d ' ' -f3 > /tmp/ethnames
  if [[ $ETH_COUNT == 2 ]]
  then 
#    echo "$i has 2 ETH links as expected. Below is the link status."
...
#      echo "$i has 1 ETH link i.e (`cat /tmp/ethnames|tr '\n' ' '`). Please check the second ETH status."
#    fi
#   ETH_STATUS=`cat $BOND_DIR/$i | grep -A1 "Slave Interface"|grep -A1 $j|grep -v $j| cut -d ":" -d ' ' -f3` 
cat $BOND_DIR/$i | grep -A1 "Slave Interface"|grep -A1 $j|egrep "up|down"| cut -d ":" -d ' ' -f3 > /tmp/ethstatus
    ...
    do
      ...
      then
        ...
        echo "$j link status showing $k in $i. No need to check this link."
      fi
     done
  done
echo
...
if [[ $ETH_STATUS == up ]] || [[ $ETH_STATUS == down ]]
then
  echo "One of the links is showing down in bonding configuration. Please verify the link status."
else
  echo "All ETH links are showing up. Bonding configuration looks good"
fi
